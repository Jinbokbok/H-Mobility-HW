...
class LidarSensorDataProcessor(Node):
    ...
    def lidar_raw_cb(self, msg):
        ...
        # 라이다 값을 전처리해서 pub함
        msg = LPFL.rotate_lidar_data(msg, offset = 0) # offset은 0부터 359까지의 값을 입력 -> 시계방향으로 몇도 돌릴 것인가. 라이다 고정 위치에 따른 영점 조절을 위해 사용하는 함수
        msg = LPFL.flip_lidar_data(msg, pivot_angle = 0) # pivot_angle은 0부터 359까지의 값을 입력 -> angle 축을 기준으로 180도 반대가 됨
